...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       						
			...
			if(orderDate && clientName && clientContact && paid && discount && paymentType && paymentStatus) {
				if(validateProduct == true && validateQuantity == true) {
	            	...
	            	'<a href="worders.php?o=add" class="btn btn-default" style="margin-left:10px;"> <i class="glyphicon glyphicon-plus-sign"></i> Add New Order </a>'+
		...
		manageOrderTable = $("#manageOrderTable").DataTable({
			'ajax': 'php_action/wfetchOrder.php',
			'order': []
		});		
...
function printOrder(orderId = null) {
	if(orderId) {		
		...
		$.ajax({
			url: 'php_action/wprintOrder.php',
			type: 'post',
			data: {orderId: orderId},
			dataType: 'text',
			success:function(response) {
				
		var mywindow = window.open('', 'Artlife Billing Management System', 'height=400,width=600');
		...
		if(productId == "") {
			...
			$.ajax({
				...
				success:function(response) {
					...
					$("#rate"+row).val(response.wrate);
					$("#rateValue"+row).val(response.wrate);

					$("#quantity"+row).val(1);
					$("#available_quantity"+row).text(response.quantity);

					var total = Number(response.wrate) * 1;
...
function removeOrder(orderId = null) {
	if(orderId) {
		$("#removeOrderBtn").unbind('click').bind('click', function() {
			...
			$.ajax({
				url: 'php_action/wremoveOrder.php',
				type: 'post',
				data: {orderId : orderId},
				dataType: 'json',
				success:function(response) {
...
function paymentOrder(orderId = null) {
	if(orderId) {
		...
		$.ajax({
			url: 'php_action/wfetchOrderData.php',
			type: 'post',
			data: {orderId: orderId},
			dataType: 'json',
			success:function(response) {				
